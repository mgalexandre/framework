...
/// connection) variant accepts a PoolConnection for use in
...
fn generate_query_function(
      ...
      <> string.join(
        list.map(list.range(1, param_count), fn(n) {
          case list.find(param_types, fn(pt) { pt.0 == n }) {
            ...
            Error(_) -> "pool_connection.string(p" <> int.to_string(n) <> ")"
          }
        }),
        ", ",
      )
  ...
  let wc_fn = case is_single_row {
    True ->
      ...
      <> row_type_name
      <> ", DbError) {\n"
      <> "  case query.select_all(connection, \""
      <> escaped_sql
      <> "\", "
      <> param_values
      <> ", "
...
/// accepts Pool, the _wc variant accepts PoolConnection. Returns
...
/// Returns the appropriate pool_connection.* wrapper function name
/// for a given column type
/// (e.g., pool_connection.int, pool_connection.string).
///
fn connection_wrapper(col_type: ColumnType) -> String {
  case col_type {
    schema_parser.Id -> "pool_connection.int"
    schema_parser.String -> "pool_connection.string"
    schema_parser.Text -> "pool_connection.string"
    schema_parser.Int -> "pool_connection.int"
    schema_parser.BigInt -> "pool_connection.int"
    schema_parser.Float -> "pool_connection.float"
    schema_parser.Boolean -> "pool_connection.bool"
    schema_parser.Timestamp -> "pool_connection.string"
    schema_parser.UnixTimestamp -> "pool_connection.int"
    schema_parser.Date -> "pool_connection.string"
    schema_parser.Json -> "pool_connection.string"
    schema_parser.Uuid -> "pool_connection.string"
    schema_parser.Foreign(_) -> "pool_connection.int"
  }
}
