...
import glimr/db/pool_connection.{
  type DbError, type PoolConnection, type Value, ConnectionError, DecodeError,
...
pub fn select(
  ...
  case pool_connection.driver(connection) {
    Postgres -> {
      let pg_connection = pool_connection.to_pog(connection)
      let pg_params = list.map(params, pool_connection.to_pog_value)
      execute_postgres_query(pg_connection, sql, pg_params, decoder)
    }
    Sqlite -> {
      let sqlite_connection = pool_connection.to_sqlight(connection)
      let converted_sql = pool_connection.convert_placeholders(sql, Sqlite)
      let sqlite_params = list.map(params, pool_connection.to_sqlight_value)
...
pub fn execute(
  ...
  case pool_connection.driver(connection) {
    Postgres -> {
      let pg_connection = pool_connection.to_pog(connection)
      let pg_params = list.map(params, pool_connection.to_pog_value)
      execute_postgres_statement(pg_connection, sql, pg_params)
    }
    Sqlite -> {
      let sqlite_connection = pool_connection.to_sqlight(connection)
      let converted_sql = pool_connection.convert_placeholders(sql, Sqlite)
      let sqlite_params = list.map(params, pool_connection.to_sqlight_value)
      execute_sqlite_statement(sqlite_connection, converted_sql, sqlite_params)
    }
  }
}
...
pub fn select_one(
  ...
  use result <- result.try(select(connection, sql, params, decoder))
  case result.rows {
    [] -> Ok(Error(Nil))
    [row] -> Ok(Ok(row))
...
fn execute_postgres_query(
  ...
  case pog.execute(query, connection) {
    ...
    Error(pog.ConstraintViolated(msg, constraint, _)) ->
      Error(pool_connection.ConstraintError(msg, constraint))
    ...
    Error(pog.QueryTimeout) -> Error(pool_connection.TimeoutError)
...
fn execute_sqlite_query(
  ...
  case sqlight.query(sql, connection, params, decoder) {
